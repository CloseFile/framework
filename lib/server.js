...
// override with different headers; last one takes precedence
  ...
  app.locals.__ = (str, params, lang) => strings.s('i18n', str, params, lang);
  app.locals.supportedLanguages = config.languages || [defaultLang];
...
// Связываем приложение
  return di('boot', config.bootstrap,
    ...
    .then((scope) => {
      // i18 init
      strings.init({translate: scope.translate});
      scope.translate.setup(lang, config.i18n || 'i18n', 'i18n');

      const themeBase = path.normalize(path.join(__dirname, '..'));
      let themePath = scope.settings.get('theme') || config.theme || 'default';
      themePath = theme.resolve(themeBase, themePath);
      const themeI18n = path.join(themePath, 'i18n');
      scope.translate.setup(lang, themeI18n, 'i18n');

      const i18nAppDir = scope.settings.get('i18n');
      scope.translate.setup(lang, i18nAppDir, 'i18n');
      scope.translate.setup(lang, i18nAppDir && path.join(i18nAppDir, 'meta'), 'meta-captions');
      return scope;
    })
    .then((scope) => {
